 ...
 */
int matchword(unsigned char *dictword, unsigned char *search, unsigned char *pattern, int flags, int length)
{
    int blanks = 0;
    register unsigned c;
    unsigned char *s;
...
int scandict(unsigned char *search, unsigned char *pattern, int flags, unsigned char *dictbuf, int length)
{
    unsigned char dictword[257] = ""; /* more than enough */
    unsigned char outword[257] = ""; /* more than enough */
    unsigned char *p = dictbuf, *e, *nextp;
    int prefixlen = 0;
    int suffixlen = 0;
    int wordcount = 0;
    while(1) // p < dictbuf + dictsize) //1)
    {
/* 	e = strchr(p, '\n'); */
/* 	if (e) { */
/* 	    nextp = e+1; */
/* 	    while (e[-1] <= ' ' && e > p)  */
/* 		e--; */
/* 	    strncpy(dictword, p, e-p); */
/* 	    dictword[e-p] = '\0'; */

        prefixlen = ((unsigned char *)p)[0];
        suffixlen = ((unsigned char *)p)[1];
        nextp = p + 2 + suffixlen;
        if (suffixlen > 0) {
            strncpy(dictword + prefixlen, p+2, suffixlen);
            dictword[prefixlen + suffixlen] = '\0';
        
            //printf("%s\n", dictword);
            strcpy(outword, dictword);
            if (matchword(outword, search, pattern, flags, length)) {
                wordcount++;
                printf("%s ", outword);
            }
        } else break;
...
int main(int argc, char *argv[])
{
    ...
    int flags;
    unsigned char *dictfile = "enable1.stm"; 
    if (argc > 1 && argv[1][0])
        dictfile = argv[1];
    if (dictfile)
    {
        ...
        fclose(dictfp);
        //BUG???!!! dictbuf[sizeof(dictbuf)-1] = '\0';
    } else {
          ...
          && fgets(inbuf, sizeof(inbuf), stdin))
    {
        ...
        for (length = l < thresh ? l : thresh; length <= l; length++) {
            printf("%d:", length);
            wordcount = scandict(wordbuf, patbuf, flags, dictbuf, length);
            if (wordcount)
                printf("(%d)| ", wordcount);
            else
                printf("-| ");
